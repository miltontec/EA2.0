...
double GlobalRandomNormal(double mean, double std) {
    ...
    if(std <= 0) {
        Print("WARNING: GlobalRandomNormal - std debe ser > 0, usando 1.0");
        std = 1.0;
    }
    ...
    hasSpare = true;
    // CORRECCIÓN #1: Evitar log(0) sumando 1.0 antes de dividir
    // AUDITORÍA: Usar 32768.0 en lugar de 32768 para precisión
    double u = (MathRand() + 1.0) / 32768.0;
    double v = (MathRand() + 1.0) / 32768.0;
    
    // AUDITORÍA: Verificar que u esté en rango válido para log
    u = MathMax(0.0001, MathMin(0.9999, u));
...
struct VotingAgent {
    ...
    void Init(string n, double w, double p, int t, double pr) {
        name = n;
        weight = w;
        performance = p;
        recentTrades = t;
...
//+------------------------------------------------------------------+
//| CORRECCIÓN #9: Función para actualizar contexto desde mercado   |
//+------------------------------------------------------------------+
void UpdateContextFromMarket(QuantumMarketContext &ctx) {
    ...
    if(avg_volume <= 0) {
        // Si no hay datos de volumen del día, usar el volumen actual como referencia
    ...
    if(atr_handle != INVALID_HANDLE) {
        ...
        if(CopyBuffer(atr_handle, 0, 0, 1, atr_buffer) > 0 && mid > 0) {
            ctx.next_hour_volatility = atr_buffer[0] / mid;  // Normalizado
        }
    ...
    ctx.data_quality_score = 100;  // Datos frescos
}

//+------------------------------------------------------------------+
//| Clase: Sistema de Votación Balanceado - AUDITADO               |
//+------------------------------------------------------------------+
class ImprovedVotingSystem {
private:
    VotingAgent agents[5];
    double adaptiveWeights[5];
    
public:
    void Initialize() {
        // CORRECCIÓN PRINCIPAL: Pesos balanceados (ML reducido de 60% a 30%)
        agents[0].Init("SR", 0.25, 0.5, 0, 0);      // Aumentado de 0.20
        agents[1].Init("ML", 0.30, 0.5, 0, 0);      // Reducido de 0.60
        agents[2].Init("Momentum", 0.20, 0.5, 0, 0);
        agents[3].Init("RSI", 0.15, 0.5, 0, 0);
        agents[4].Init("Volume", 0.10, 0.5, 0, 0);
        
        // AUDITORÍA: Inicializar adaptiveWeights
        for(int i = 0; i < 5; i++) {
            adaptiveWeights[i] = agents[i].weight;
        }
    }
    
    void UpdatePerformance(int agentId, bool success, double profit) {
        // AUDITORÍA: Validación estricta de rango
        if(agentId < 0 || agentId >= 5) {
            Print("ERROR: UpdatePerformance - agentId fuera de rango: ", agentId);
            return;
        }
        
        agents[agentId].recentTrades++;
        agents[agentId].recentProfit += profit;
        
        // Actualizar performance con decaimiento
        double alpha = 0.1;  // Factor de aprendizaje
        agents[agentId].performance = 
            (1 - alpha) * agents[agentId].performance + 
            alpha * (success ? 1.0 : 0.0);
        
        // AUDITORÍA: Clamp performance to valid range
        agents[agentId].performance = MathMax(0.0, MathMin(1.0, agents[agentId].performance));
        
        // Ajustar pesos dinámicamente
        if(agents[agentId].recentTrades >= 10) {
            double winRate = agents[agentId].recentProfit > 0 ? 0.6 : 0.4;
            agents[agentId].weight *= (0.9 + 0.2 * winRate);
            
            // AUDITORÍA: Limitar peso a rango válido antes de normalizar
            agents[agentId].weight = MathMax(0.05, MathMin(2.0, agents[agentId].weight));
            
            // Normalizar pesos
            NormalizeWeights();
            
            // Reset counters
            agents[agentId].recentTrades = 0;
            agents[agentId].recentProfit = 0;
        }
    }
    
    ConsensusResult BuildConsensus(double &buyVotes[], double &sellVotes[], int voteCount) {
        ConsensusResult result;
        result.isValid = false;  // AUDITORÍA: Inicializar explícitamente
        result.direction = 0;
        result.strength = 0.0;
        result.confidence = 0.0;
        
        // AUDITORÍA: Validar arrays de entrada
        if(ArraySize(buyVotes) == 0 || ArraySize(sellVotes) == 0 || voteCount <= 0) {
            Print("ERROR: BuildConsensus - Arrays de votos inválidos");
            return result;
        }
        
        double buyScore = 0, sellScore = 0;
        
        // AUDITORÍA: Usar mínimo entre voteCount y tamaño real de arrays
        int maxVotes = MathMin(5, MathMin(voteCount, MathMin(ArraySize(buyVotes), ArraySize(sellVotes))));
        
        // Recopilar votos ponderados por performance
        for(int i = 0; i < maxVotes; i++) {
            // AUDITORÍA: Validar performance antes de usar
            double effectiveWeight = agents[i].weight * MathMax(0.1, agents[i].performance);
            
            buyScore += buyVotes[i] * effectiveWeight;
            sellScore += sellVotes[i] * effectiveWeight;
        }
        
        // AUDITORÍA: Prevenir división por cero
        double totalScore = buyScore + sellScore;
        if(totalScore <= 0) {
            Print("WARNING: BuildConsensus - totalScore es 0");
            return result;
        }
        
        double differential = MathAbs(buyScore - sellScore) / totalScore;
        
        result.isValid = (differential > 0.2);  // 20% de diferencia mínima
        result.direction = (buyScore > sellScore) ? 1 : -1;
        result.strength = MathMin(1.0, differential);  // AUDITORÍA: Limitar a 1.0
        result.confidence = MathMin(1.0, totalScore / 5.0);
...
private:
    void NormalizeWeights() {
        double sum = 0;
        for(int i = 0; i < 5; i++) {
            // AUDITORÍA: Solo sumar pesos válidos
            if(agents[i].weight > 0) {
                sum += agents[i].weight;
            }
        }
        ...
        if(sum <= 0) {
            Print("ERROR: NormalizeWeights - suma de pesos es 0, reseteando a valores por defecto");
            for(int i = 0; i < 5; i++) {
                agents[i].weight = 0.2;  // Distribución uniforme
            }
            return;
        }
        ...
        for(int i = 0; i < 5; i++) {
            agents[i].weight /= sum;
            // AUDITORÍA: Asegurar peso mínimo
            agents[i].weight = MathMax(0.05, agents[i].weight);
        }
    }
};
...
private:
    ...
    bool ExecuteSingle(const QuantumDecisionProfile &decision) {
        Print("🔹 EJECUTANDO SINGLE ORDER");
        
        double volume = CalculateVolume(decision.risk_per_order[0], 0.0);
        ...
        if(decision.direction > 0) {
            result = m_trade.Buy(volume, _Symbol, price, sl, tp, 
                                decision.reasoning);
        } else {
            result = m_trade.Sell(volume, _Symbol, price, sl, tp, 
                                 decision.reasoning);
        }
        
        if(result) {
            RegisterExecution(m_trade.ResultOrder(), price, sl, tp, 
                            decision.use_ai_trailing);
            Print("✅ Orden ejecutada: ", m_trade.ResultOrder());
        }
    ...
    bool ExecuteScaleIn(const QuantumDecisionProfile &decision) {
        ...
        Print("📊 EJECUTANDO SCALE-IN - ", orders_count, " órdenes");
        ...
        for(int i = 0; i < orders_count; i++) {
            ...
            // Delay óptimo entre órdenes
            if(i > 0) Sleep(CalculateOptimalDelay(i, decision));
        ...
        Print("Scale-in completado: ", success_count, "/", 
              orders_count, " órdenes ejecutadas");
...
//+------------------------------------------------------------------+
//| Clase Principal: Sistema de Quantum Neural Ensemble            |
//+------------------------------------------------------------------+
...
private:
    ...
    ImprovedVotingSystem m_voting_system;
...
public:
    ...
    void Initialize() {
        Print("Inicializando Quantum Neural Ensemble...");
        ...
        Print("✅ Quantum Neural Ensemble inicializado completamente");
    }

    // Overload con configuración externa y valor de retorno para chequeo
    bool Initialize(const int memory_size, const int ensemble_size) {
        // Se aceptan parámetros pero el sistema usa configuraciones internas por ahora.
        // Guardamos valores por si se usan en futuras optimizaciones.
        Print("Inicializando Quantum Neural Ensemble (configurable)...");
        // Inicialización base
        Initialize();
        // Marcar como inicializado y devolver éxito
        return m_initialized;
    }
    
    QuantumDecisionProfile MakeQuantumDecision(
        const QuantumMarketContext &context
    ) {
        QuantumDecisionProfile decision;
        decision.Initialize();
        ...
        // 1. LSTM predice serie temporal
        double lstm_forecast[];
        m_lstm.PredictSequence(context, lstm_forecast, 24);
        
        // 2. Transformer analiza importancia de features
        double votes_matrix[5][10];
        for(int i = 0; i < 5; i++) {
            for(int j = 0; j < 10; j++) {
                votes_matrix[i][j] = GlobalRandomNormal(0, 1);
            }
        }
        
        double attention_weights[];
        
        // Convertir matriz a formato esperado
        double flat_matrix[][10];
        ArrayResize(flat_matrix, 5);
        for(int i = 0; i < 5; i++) {
            for(int j = 0; j < 10; j++) {
                flat_matrix[i][j] = votes_matrix[i][j];
            }
        }
        
        m_transformer.ComputeAttention(flat_matrix, attention_weights);
        
        // 3. GAN genera escenarios adversos
        double worst_case_scenarios[];
        m_gan.GenerateAdversarialScenarios(context, worst_case_scenarios);
        
        // 4. Meta-learner combina todo con RL
        decision = m_metalearner.SynthesizeDecision(
            lstm_forecast,
            attention_weights,
            worst_case_scenarios
        );
        
        // 5. Aplicar ajustes cuánticos
        ApplyQuantumAdjustments(decision, context);
        
        // 6. Generar forecast completo
        QuantumForecast forecast = m_forecast_engine.GenerateQuantumForecast(context);
        
        // 7. Integrar forecast en decisión
        IntegrateForecast(decision, forecast);
        
        // 8. Verificar con sistema de aprendizaje
        ...
        if(successProbability < 0.4) {
            Print("⚠ Probabilidad de éxito baja (", successProbability, ") - Reduciendo agresividad");
    ...
    // Exponer forecast completo para uso externo (e.g., TradingStrategy)
    QuantumForecast GenerateQuantumForecast(const QuantumMarketContext &context) {
        return m_forecast_engine.GenerateQuantumForecast(context);
    }
    
    void UpdateLearning(int agent_id, bool success, double profit, 
                       const QuantumMarketContext &context) {
...
private:
    void ApplyQuantumAdjustments(
        ...
        if(context.smart_money_flow * decision.direction > 0.8) {
            if(decision.action < ACT_EXEC_PYRAMID) {
                int new_action = (int)decision.action + 1;
    ...
    void IntegrateForecast(QuantumDecisionProfile &decision, 
                          const QuantumForecast &forecast) {
        // Ajustar decisión basada en forecast
        if(forecast.h1_direction_prob > 0.7 && decision.action >= ACT_EXEC_SINGLE) {
            decision.primary_orders = MathMin(decision.primary_orders + 1, 
                                             QUANTUM_MAX_ORDERS);
        }
...
// ================================================================
// Minimal wrapper MetaLearningSystem providing the API used by
// TradingStrategy.mq5 and VotingStatistics.mqh.
// ================================================================
struct AgentStats {
    int    trades;
    int    wins;
...
public:
    ...
    // --- API expected by VotingStatistics.mqh ---
    double GetAgentWeight(int agent_id) {
        // Lightweight proxy using the weight system with a neutral context
        QuantumMarketContext ctx;
        ctx.Initialize();
        UpdateContextFromMarket(ctx);  // Usar contexto real
        
        // Aplicar ajustes de error patterns si existen
        double baseWeight = m_ensemble.GetAgentQuantumWeight(agent_id, ctx);
        
        // Buscar ajustes de error patterns activos
        for(int p = 0; p < m_errorPatternCount; p++) {
            if(m_errorPatterns[p].agentName == m_agentNames[agent_id]) {
                baseWeight *= m_errorPatterns[p].adjustmentFactor;
            }
        }
